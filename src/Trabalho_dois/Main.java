...
public class Main {
	...
	public static int[] subTabela(int[] tabela) {
		...
		for(int i = 0; i<tabela.length; i++) {
			...
			//o proximo termo n�o � maior que o anterior, logo está desordenada nestes indexes
			if(tabela[i] > tabela[i+1]) {
				if(tabelaordenada == true) //� a primeira vez que estou a passar aqui porque estava true e vai passar a n�o estar.
					sepindex2= i;
		...
		//ver exce��o no caso do utlimo numero desordenado seja antes da primeiro desordenado.
		for(int i = 0; i<tabela.length; i++) {
		...
		//System.out.printf("Nr de valores: %d, Come�o(sepindex2): %d, Fim(sepindex): %d\n",sepindex-sepindex2+1,sepindex2,sepindex);
